...
class HTMLGen
{
	...
	public static function generateHTMLList( $filepath, $namespace, $database, $table, $fields = null )
	{
		...
		$file = FileSystemService::writeFile ( TemplateManager::$SERVER_OUTPUT . $folderNamespace . '/html/' . $filename, $template );
		
		Utilities::log( $database.'-HTML-LIST', 'wrote '.$tableUFirst . 'List.php to '.TemplateManager::$SERVER_OUTPUT . $folderNamespace );
	...
	public static function generateHTMLForm( $filepath, $namespace, $database, $table, $fields = null )
	{
		...
		*/
		foreach ( $fields as $field )
		{
			...
			$fieldFormFields .= '
			<p>
		 		<label for="txt_'.$fName.'">'.ucfirst( $fName ).'</label>
		 		<input id="txt_'.$fName.'" name="'.$fName.'" value="<?php echo $'.$fName.'; ?>" type="text"/>
				<span class="subtext">'.$fType.'</span>
		 	</p>';
		...
		Utilities::log( $database.'-HTML-Form', 'wrote '.$tableUFirst . 'Form.php to '.TemplateManager::$SERVER_OUTPUT . $folderNamespace );
	...
	public static function generateHTMLMain( $filepath, $namespace, $database, $table, $fields = null )
	{
		...
		foreach ( $table as $tbl )
		{
			...
			$tableLinks .= '
			<a href="' . $tableUFirst . 'List.php">' . $tableUFirst . ' </a>
			';
		}
	...
	//	FileSystemService::recursiveCopy( TemplateManager::$HTML_TEMPLATE_LOCATION.'assets', TemplateManager::$SERVER_OUTPUT . $folderNamespace . '/html/assets/' );
		...
		$header = preg_replace ( TemplateManager::$HTML_TABLE_LIST_PATTERN, $tableLinks, $header );
		
		FileSystemService::writeFile ( TemplateManager::$SERVER_OUTPUT . $folderNamespace . '/html/' . '_header.php', $header );
		FileSystemService::writeFile ( TemplateManager::$SERVER_OUTPUT . $folderNamespace . '/html/' . '_footer.php', $footer );
		FileSystemService::writeFile ( TemplateManager::$SERVER_OUTPUT . $folderNamespace . '/html/' . 'styles.css', $css );
		FileSystemService::writeFile ( TemplateManager::$SERVER_OUTPUT . $folderNamespace . '/html/' . 'jquery.js', $js );
		
		$mainFilepath = TemplateManager::$SERVER_OUTPUT . $folderNamespace . '/html/' . $filename;
		
		$file = FileSystemService::writeFile ( $mainFilepath, $template );
		
		Utilities::log( $database.'-HTML-Main', 'wrote index.php to '.TemplateManager::$SERVER_OUTPUT . $folderNamespace );
